...
# read in a file with 1 alignment, 
# eliminate alignment columns with too many gaps (default 80% non-gaps required
# get tree using FastTree 
# reroot using urec.
...
GetOptions(
	   ...
	   'nj!'          => \$do_nj, # excalmation point means can use  -nj  and  -nonj
	   'ml!'      => \$do_ml,
...
my $n_ml = 0;
$idline = '(alignment has no id line)';
while (<>) {
  ...
  if ( $state eq 'idline' ) {
    ...
    $state  = 'fasta';
    $fasta  = '';
  } elsif ( $state eq 'fasta' and /\S/) {
    # not an empty line - append to $fasta
    $fasta .= $_;
  }
}


if (0 or /^\s*$/) {	  # blank line after sequence -> process the sequence.
  chomp $idline;
