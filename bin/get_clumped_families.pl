...
GetOptions(
	   ...
	   'gg_file=s'           => \$gg_filename, #
           'use_clade_seqs!' => \$use_clade_seqs, # use_clade_seqs or nouse_clade_seqs
          );
if (defined $abcfilelist) { # store abc filenames in hash (key: species; value: filename
   open my $fhlist, "<", "$abcfilelist" or die "couldn't open $abcfilelist for reading. exiting.\n";
print "reading species abc files from $abcfilelist \n";
   %species_abcfile = ();
   for (<$fhlist>) {
      if (/^\s*(\S+)\s+(\S+)\s*$/) {
      #   print "$1 $2 XXX \n";
...
my $gg_hashref = store_gg_info($gg_filename); # store seq_id:species pairs in hash


# read connected components (output of clumper.pl) 
open my $fhin, "<", "$clump_qid_filename" or die "couldn't open $clump_qid_filename for reading.\n";
my %clumpidnumber_qidset = (); # query ids lumped together (1 for each member of connected component.)
my %clumpidnumber_clamidset = (); # sequences in am clade of at least one of the lumped-together families.
my %qid_clumpidnumber = ();
my $clump_id_number = 1;
while (my $idline = <$fhin>) { # read info about a connected component
   next if($idline =~ /^\s*#/);
   my @cols = split(" ", $idline);
   my ($n_qids_clumped, $clump_qids_string, $clump_amcladeids_string) = @cols[3,9,11]; # size (# of query ids), qids, all amclade ids, or a connected component.
   my @clump_qids = split(",", $clump_qids_string); #print join("; ", @clump_qids), "\n";
   $clump_amcladeids_string =~ s/([|])//g;
   my @clump_amcladeids = split(",", $clump_amcladeids_string); # print join("; ", @clump_amcladeids), "\n"; exit;
...
while(my($s,$f) = each %species_abcfile){
   print "$s  $f  \n";
}
...
while (my($sp,$abc_filename) = each %species_abcfile) {
   ...
   #   print "$id1 $id2 $ev \n";
      ...
      #   print "$id1 is in clump  $clump_id_number \n";
       ...
       #  print "$id1  is not query of any of lumped-together fams.\n";
      }
      if ($use_clade_seqs) {
         for my $clumpid (keys %clumpidnumber_clamidset) {
            if (exists $clumpidnumber_clamidset{$clumpid}->{$id1}) {
               if (exists $clumpidnumber_allidset{$clumpid}) {
                  $clumpidnumber_allidset{$clumpid}->{$id2}++;
               } else {
                  $clumpidnumber_allidset{$clumpid} = {$id2 => 1};
               }
            }
         }
      }
...
print STDERR "# how many clump numbers? ", scalar @sorted_clump_numbers, "\n";
