...
my $first_line = <>;
#print STDERR "first line $first_line \n";
$first_line =~ s/\s+$//;
...
while (<>) {
  ...
  # }
  my $type = shift @cols;
  if ($n_clades >= 5 and $cols[4*$columns_per_clade] < 0) {
   $cols[4*$columns_per_clade] = 10000;
  }
# print "XXX: $columns_per_clade. $n_clades; ", join(" ", @cols), "\n";
  my @depths = ();
  my @nparalogs =();
  my @ndisallowed = ();
  my @sizes = ();
...
sub check_nesting{
  ...
  for (@nesting) {
    my ($inner, $outer);
    if (/ (\d+) \s* ( [<] | lt ) \s* (\d+) /x) {
      ($inner, $outer) = ($1-1, $3-1); # split(/ \s* ( [<] | lt ) \s* /x, $_);
      #    print STDERR "in/out: $inner $outer \n";
