...
my ($AMpos1023_offset, $AMpos_offset, $monocots_offset, $non_angiosperms_offset, $AMneg_offset) = 
  (0, 5, 10, 15, 20);
...
#print "strict nesting? $strict_nesting \n";
# printf("%6.4f %6.4f %6.4f %6.4f \n",
#	     $Pdicots_in_Monocots_support, $Monocots_in_Nonangiosperms_support, 
#	     $Pdicots_in_Nonangiosperms_support, $Monocots_in_Negatives_support);
#print "node heights:  $monocots_node_height  $AMneg_node_height \n";
#max( ($AMneg_eps_prod > 0)? 1 - $AMneg_eps_prod/$monocots_eps_prod : 1, -1);

    chomp;

    my $min_supp1 = min($Pdicots_in_Monocots_support, 
# $Monocots_in_Basals_support, 
$Monocots_in_Negatives_support);
...
# print "[", ($AMneg_node_height < 0 or $AMneg_node_height > $monocots_node_height), "]", "\n";
#      print "$AMneg_node_height  $monocots_node_height \n";
# print "[", ($nonangiosperms_node_height < 0 or $nonangiosperms_node_height > $monocots_node_height), "]", "\n";
# print "[", ($nonangiosperms_node_height < 0 or $nonangiosperms_node_height > $AMposdicots_node_height)
    }
...
sub a_in_b_support{
  my $a_eps_prod = shift;
  my $b_eps_prod = shift;
  if ($a_eps_prod == -1) { # a absent
    return 0;
  } else {
    if ($b_eps_prod == -1) { # b absent
      return 1;
    } else {
...
sub a_in_b_support_inner{	    # returns value indicating support for
  ...
  # returns a value 0<x<1 describing support for possible nestings:
