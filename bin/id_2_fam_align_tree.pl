...
# given sequence id, cluster file, sequences in fasta file, and gg file,
# get the cluster containing the sequence id, get the sequences (as fasta) for
# all sequences in cluster, align the sequences, construct tree with FastTree.
...
#print "[[[ $FT_out_newick ]]]\n";
   ...
   $FT_out_filename = $align_filename;
$FT_out_filename =~ s/[.]fasta$//;
$FT_out_filename =~ s/_align//;
$FT_out_filename =~ s/_tax//;
$FT_out_filename .= '_FT_out.newick';
open my $fh_FTout, ">", "$FT_out_filename";
    print $fh_FTout "$FT_out_newick \n";
close $fh_FTout;
print "$FT_out_filename   $seqid0  \n";
  }else{
     $FT_out_filename = `FT.pl $align_filename 0 `;
  }
  print "$seqid0, $FT_out_filename  \n";
print "[$clade_outer_edge]  [$other] \n";
my $reroot_cl = " ~/Orthologger/bin/reroot_prune.pl ";
$reroot_cl .= " -input $FT_out_filename  -reroot mindl  -sequence_id  $seqid0 ";
$reroot_cl .= " -clade_outer_edge $clade_outer_edge -other $other ";

print "ZZZZ: $reroot_cl \n";
my $reroot_prune_out = ` $reroot_cl `; 
#`~/Orthologger/bin/reroot_prune.pl -input $FT_out_filename  -reroot mindl  -sequence_id  $seqid0 -clade_outer_edge $clade_outer_edge -other $other`; # 'Medtr7g069610.1'`;  
#  }
}
# exit;
...
# ############################# PRUNING ####################################
# my $non_angiosperm_species = { 'Chlamydomonas_reinhardtii' => 1,
...
#       my @coe_species = split("[, ]+", $clade_outer_edge);
#       for (@coe_species) {
# 	$user_outer_edge_species->{$_} = 1;
#       }
#       $clade_outer_edge_species = $user_outer_edge_species;
#     }
#     print "clade outer edge species: \n";
#     for (keys %{$clade_outer_edge_species}) {
#       print "$_  ", $clade_outer_edge_species->{$_}, "\n";
#     }

#     my $other_species;
#     my @oth_species = split("[, ]+", $other);
#     for (@oth_species) {
#       $other_species->{$_} = 1;
#     }
#     print "other species to count in and out of clade: \n";
#     for (keys %{$other_species}) {
#       print "$_  ", $other_species->{$_}, "\n";
#     }
#     my ($pruned_tree_root, $in_other, $out_other) = $tree->min_clade($seqid0, $prune_threshold, $clade_outer_edge_species, $other_species);
# print "other counts; in: $in_other, out: $out_other \n";
