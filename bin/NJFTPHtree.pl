...
GetOptions(
	   ...
	   #	   'ft_options=s' => \$additional_ft_options,
           'n_best_trees_to_output=i' => \$n_best_trees_to_output,
	  );
         ...
         if ($fasta ne '') {
            ...
            if ( $overlap_length >= $min_overlap_length and $overlap_length <= $max_overlap_length and $overlap_obj->get_n_sequences() < $max_n_sequences) { # sufficient overlap
               ...
               for my $i_bs (1..$n_bs) {
                  ...
                  print STDERR "$type $bsnjft_newick \n";
...
#                  print "ZZZZ: \n", $skeys[0], "\n";
                  if ($i <= $n_best_trees_to_output) {
                     if (!defined $phymlobj) {
                        print $fh_out ("Id $qid  $descript  $ft_lnL  $delta_lnL  $ft_cputime   -  0  0  \n");
                        if ($reroot_method eq 'mindl') {
                           my $taxonified_newick =
                             taxonify_newick( $newick, $gg_hashref );
                      #     print STDERR "TAX: $taxonified_newick \n";
                           my    $rr_tax_newick =
                             newick2rerootednewick( $taxonified_newick,
                                                    $reroot_method, $species_tree );
                     #      print STDERR "REROOTED: $rr_tax_newick \n";

                           print $fh_out "$descript  $rr_tax_newick \n\n";
                        } else {
                           print $fh_out "$descript  ", newick2rerootednewick( $newick,
                                                    $reroot_method, $species_tree ), "\n\n";
                        }
                     } else {
